defmodule Aurora.Uix.Templates.Basic.Actions.ManyToMany do
  ...
  represents a many-to-many membership: beside the label, at the top right, and below the list.

  ## Key Features

    - Adds `:default_toggle_all`, a tri-state checkbox rendered beside the group label: checked when
      every candidate is a member, unchecked when none is, and a dash when only some are. Clicking a
      checked toggle clears the membership; clicking it in either other state selects everything.
  ...
  ## Key Constraints
    ...
    - The toggle rides the parent form's `phx-change="validate"` and is identified by `_target`
      ...
      the pre-click membership and undo the toggle.
    - Its input name is deliberately outside the resource scope, so the toggle never reaches the
      host's changeset or Ash action as an unknown key.
    - Expects the `assigns` map to include an `:auix` key with `:layout_type`,
      `:many_to_many_toggle_state` and the layout tree options, plus the `:field` being rendered.
  """
  ...
  - `assigns` (map()) - Assigns map containing the association context.
    - Must include `:auix` (with `:layout_type` and `:many_to_many_toggle_state`) and `:field`.

  ## Returns
  Phoenix.LiveView.Rendered.t() - Rendered checkbox component.
  """
  @spec toggle_all(map()) :: Rendered.t()
  def toggle_all(%{auix: %{layout_type: :form, many_to_many_toggle_state: state}} = assigns) do
    ...
    ~H"""
    <.input
      id={"auix-many-to-many-toggle_all-#{@field.key}"}
      name={"auix_toggle_all__#{@field.key}"}
      type="checkbox"
      label=""
      value={@checked?}
      input_class={@mixed_class}
      title={dt("Select or clear every record")}
    />
    """
  end
  ...
  defp add_default_label_actions(assigns) do
    Actions.add_actions(assigns, :many_to_many_label_actions, default_toggle_all: &toggle_all/1)
  end

  @spec add_default_header_actions(map()) :: map()
  defp add_default_header_actions(%{auix: %{many_to_many_header_actions: _}} = assigns),
    do: assigns

  # Adds an empty many_to_many_header_actions list if not present. The library ships no default
  ...
  defp add_default_header_actions(assigns),
    do: put_in(assigns, [:auix, :many_to_many_header_actions], [])
