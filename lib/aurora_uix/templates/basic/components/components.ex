defmodule Aurora.Uix.Templates.Basic.Components do
        ...
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="auix-items-table-row">
            <td
              ...
              class={"auix-items-table-cell " <> if @row_click, do: "auix-items-table-cell-clickable", else: ""}
            >
              <div class="auix-items-table-cell-content">
                <span class="auix-items-table-cell-focus-ring" />
                <span class={"auix-items-table-cell-text " <> if i == 0, do: "auix-items-table-cell-text-first", else: ""}>
    ...
    ~H"""
      <div class="pagination-bar">
        ...
        <%= if @pages_start_index > 1 do %>
          <a name="auix-pages_bar_page-first" class="pagination-bar-link" phx-click="pagination_to_page" phx-value-page={1}>
            <span>1</span>
        ...
        <div :for={page_index <- @pages_start_index..@pages_end_index}>
          <%= if page_index == @pagination.page do %>
            <div class="pagination-bar-current-page">
              <span name="auix-pages_bar_page-current" class="pagination-bar-current-page-number">
                {page_index}
              </span>
              <.selected_count selected_in_page={@selected_in_page} page={page_index} />
            </div>
          <% else %>
            <a name={"auix-pages_bar_page-#{page_index}"} class="pagination-bar-link" phx-click="pagination_to_page" phx-value-page={page_index}>
        ...
        <%= if @pages_end_index < @pagination.pages_count do %>
          <a name="auix-pages_bar_page-right"class="pagination-bar-link" phx-click="pagination_to_page" phx-value-page={@pages_right_index}>
            <span>...</span>
            <.selected_count selected_in_page={@selected_in_page} from={@pages_end_index} to={@pagination.pages_count} />
          </a>
          <a name="auix-pages_bar_page-last" class="pagination-bar-link" phx-click="pagination_to_page" phx-value-page={@pagination.pages_count}>
  ...
  defp selected_count(%{selected_in_page: selected_in_page, from: from, to: to} = assigns) do
    ...
    ~H"""
    <%= if @count > 0 do %>
      <span class="pagination-bar-selected-count">
        {@count}
      </span>
    <% end %>
    """
