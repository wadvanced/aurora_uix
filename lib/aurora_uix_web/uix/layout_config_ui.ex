defmodule AuroraUixWeb.Uix.LayoutConfigUI do
  ...
  ```elixir
    generate_layouts([], %{product: %{fields: [:name, :price]}})
  ```
  """
  @spec generate_layouts(list, map) :: list
  def generate_layouts(resource_configs, form_layouts) do
    resource_configs
    |> extract_config_field_names()
    |> do_generate_layouts(resource_configs, form_layouts)
  end
  ...
  @spec do_generate_layouts(map, list, map | nil) :: list
  defp do_generate_layouts(fields, resource_configs, form_layouts),
    do: Enum.map(resource_configs, &generate_layout(&1, fields, form_layouts))

  @spec generate_layout(tuple, map, list) :: tuple
  defp generate_layout({data_name, resource_config}, fields, form_layouts) do
