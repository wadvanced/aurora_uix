defmodule Aurora.UixWeb.Test.BrowserEmbedsManyTest do
  ...
  feature "Create a new entry without embeds many ", %{session: session} do
    ...
    page = visit(session, "/browser-embeds-many-users")
  ...
  feature "Create a new entry with embeds many ", %{session: session} do
    ...
    # Checks if it is well shown
    session
    |> visit("/browser-embeds-many-users")
    |> assert_text("Listing Users")
    |> click_and_wait(@new_button, @save_button)
  ...
  defp added_entry(field, index) do
    Query.css("[name='user[emails][#{index}][#{field}]']", visible: false)
  end
