...
- `:ash_actor_assign` (atom()) - Optional. Name of the `socket.assigns` key that
  ...
  receives `actor: socket.assigns[<assign>]` — see
  [Ash Integration → Authorization & policies](ash_integration.md#authorization--policies)
  for the full behaviour. `:actor_assign` is accepted as an alias.
