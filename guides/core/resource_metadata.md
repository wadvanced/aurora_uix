...
- `:ash_actor_assign` (atom()) - Optional. Name of the `socket.assigns` key that
  holds the actor used to authorize policy-protected resources. Default `nil`
  (no `actor:` forwarded). When set, every CRUD call generated by Aurora UIX
  receives `actor: socket.assigns[<assign>]`. `:actor_assign` is accepted as an
  alias.
  ...
  See [Ash Integration → Authorization & policies](ash_integration.md#authorization--policies)
  for the worked example, troubleshooting checklist, and the full behaviour
  matrix.
